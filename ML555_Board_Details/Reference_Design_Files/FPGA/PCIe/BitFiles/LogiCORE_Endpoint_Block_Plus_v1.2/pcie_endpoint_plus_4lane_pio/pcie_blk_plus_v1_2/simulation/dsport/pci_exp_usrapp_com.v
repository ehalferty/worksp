...
initial begin
  ...
  *************************************************************/
  ...
  TSK_DECIPHER_FRAME (fmt, type, traffic_class, td, ep, attr, length, log_file);  
  ...
  casex({fmt, type})
    ...
    `PCI_EXP_MEM_READ32 : begin
      ...
      payload = 0;
      TSK_3DW(fmt, type, traffic_class, td, ep, attr, length, payload, _frame_store_idx, _log_file_ptr, log_file);
    ...
    `PCI_EXP_MEM_READ64: begin
      ...
      payload = 0;
      TSK_4DW(fmt, type, traffic_class, td, ep, attr, length, payload,  _frame_store_idx, _log_file_ptr, log_file);
  ...
  Inputs : None
  Outputs : fmt, type, traffic_class, td, ep, attr, length
  ...
  *************************************************************/
  ...
  begin
    ...
    type = txrx ? frame_store_tx[0] : frame_store_rx[0];
    ...
    reg  [3:0]  _rem;
      ...
      for (_i = 0; _i < _rem; _i = _i + 1) begin
        ...
        if (txrx) begin
          ...
          `BOARD.`XILINX_PCI_EXP_DOWNSTREAM_PORT_INST.com_usrapp.frame_store_tx[`BOARD.`XILINX_PCI_EXP_DOWNSTREAM_PORT_INST.com_usrapp.frame_store_tx_idx] = _byte;
          `BOARD.`XILINX_PCI_EXP_DOWNSTREAM_PORT_INST.com_usrapp.frame_store_tx_idx = `BOARD.`XILINX_PCI_EXP_DOWNSTREAM_PORT_INST.com_usrapp.frame_store_tx_idx + 1;

        end else begin

          `BOARD.`XILINX_PCI_EXP_DOWNSTREAM_PORT_INST.com_usrapp.frame_store_rx[`BOARD.`XILINX_PCI_EXP_DOWNSTREAM_PORT_INST.com_usrapp.frame_store_rx_idx] = _byte;
          `BOARD.`XILINX_PCI_EXP_DOWNSTREAM_PORT_INST.com_usrapp.frame_store_rx_idx = `BOARD.`XILINX_PCI_EXP_DOWNSTREAM_PORT_INST.com_usrapp.frame_store_rx_idx + 1;
        end
